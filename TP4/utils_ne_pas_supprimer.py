...
class TestCaseWholeFile(unittest.TestCase):
    tests_case = []
    file_to_run = dict()
    def __init__(self, test_name: str, filename: str, mock_input: List, expected_output: List, timeout: float = 1, fail_message: str = None):
    ...
    def run_whole_file(self):
        if self.file not in TestCaseWholeFile.file_to_run:
            TestCaseWholeFile.file_to_run[self.file] = importlib.import_module(self.file)
        else:
            importlib.reload(TestCaseWholeFile.file_to_run[self.file])
